...
from app.utils import in_last24h, normalize_datetime, utc_now, validation_error_response
...
@bp.get("/<int:log_id>")
@jwt_required()
def get_climate_log(log_id: int):
    db = SessionLocal()
    try:
        ...
        # 与 dashboard 的 climateLast24h 同源：同一套 UTC 滑窗
        payload["inLast24h"] = in_last24h(item.recorded_at)
